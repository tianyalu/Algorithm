 ...
 *      如果有足够空间，计算出需要的空间。根据最终需要的总空间，维护一个指针在最后，从后往前，
 *    遇到非空的就把该值挪到指针指向的位置，然后指针向前一位；遇到“ ”，则指针前移，依次替换为“02%”。
 *
 * @Author: tian
 * @UpdateDate: 2021/2/4 10:15 AM
 */
public class ReplaceBlank {
    public static void main(String[] args) {
        ...
        copyCharArray(str, newStr);

        int usedLength = replaceBlank(newStr, str.length);

        printCharArray(newStr, usedLength);
    }
    ...
    private static void copyCharArray(char[] src, char[] dst) {
        if(src == null || src.length == 0 || dst == null || dst.length == 0) {
            return;
        }

        int targetLen = Math.min(src.length, dst.length);
        for (int i = 0; i < targetLen; i++) {
            dst[i] = src[i];
        }
    }

    private static void printCharArray(char[] str, int usedLength) {
        if(str == null || str.length == 0) {
            return;
        }

        int targetLength = Math.min(str.length, usedLength);
